...
#extraData=$($EXTRADATAGEN_BINARY $(echo "${addresses[@]}" | tr -d ' '))
validators=$(echo "${addresses[@]}")
extraData=$($EXTRADATAGEN_BINARY "$validators") 
...
echo "Over writing node key with validator key"
echo "======================="
for i in {0..3}; do
     data_dir="$NETWORK_DIR/signer-$i"
     keystore_file=$(find ./scripts/local/network/signer-$i/keystore/U*)
     keystore_dump=$($ETHKEY_BINARY inspect --passwordfile "$pw_file" --private $keystore_file)
     echo $keystore_dump | awk -F" " '//{print $8}' > $data_dir/geth/nodekey
done
